...
L = gen()
for i in range(len(L)):
    P = [point_1, P[L[i][0]-1], P[L[i][1]-1], P[L[i][2]-1], P[L[i][3]-1], point_1]
    for j in range(5):
...
for i in range(5):
    P = [point_1, P[L[number_short_route][0]-1], P[L[number_short_route][1]-1], \
        P[L[number_short_route][2]-1], P[L[number_short_route][3]-1], point_1]
    C = C + rang(P[i][0], P[i][1], P[i+1][0], P[i+1][1])
    print(C)


print(f"There are {sum1.count(short_route)} shortest route, their length = {short_route} m.")

print(L[number_short_route])
